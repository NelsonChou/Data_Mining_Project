...
freq_names_dfm <- dfm_trim(names_dfm, min_count = 30)
...
#name
class(freq_names_dfm) <- class(sparse_matrix)
#24gram
...
fullycombined12 <- cbind(
  sparse_matrix, # basic features
  freq_dfm12,  # description
  freq_names_dfm)
fullycombined24 <- cbind(
  sparse_matrix, # basic features
  freq_dfm24,  # description
  freq_names_dfm)

rownames(fullycombined12) <- NULL
rownames(fullycombined24) <- NULL
head(fullycombined12,5)

sparse_train12 <- fullycombined12[seq_len(nrow(train)), ]
sparse_test12  <- fullycombined12[seq(from = (nrow(train) + 1), to = nrow(fullycombined12)), ]
sparse_train24 <- fullycombined24[seq_len(nrow(train)), ]
sparse_test24  <- fullycombined24[seq(from = (nrow(train) + 1), to = nrow(fullycombined24)), ]
...
saveRDS(modelng12,file="/Users/douzhi/Desktop/Fall Mod1 2018/MGMT 571 Data Mining/Project/modelng12_rds.rds")
